...
- K-045 — /about desktop layout consistency（2026-04-24，Architect 設計完成，Engineer 未交付）：BQ-045-02 裁決 Option α — `frontend/src/components/primitives/SectionContainer.tsx` 退役（單一 consumer + byte-identity target with HomePage.tsx，Reviewer parity gate reduced to string-compare）；`AboutPage.tsx` 預期改寫為 6 `<section>` 直接為 root `<div className="min-h-screen">` 子元素（pattern A per ticket §4a，避免 #architecture.nextElementSibling 斷開），每 section inline `max-w-[1248px] mx-auto px-6 sm:px-24 w-full` + `mt-6 sm:mt-[72px]`（S2–S6）/ `pt-8 sm:pt-[72px]`（S1 hero）/ `mb-8 sm:mb-[96px]`（S6 before footer）；對齊 Pencil frame 35VCj `Y80Iv padding:[72,96,96,96], gap:72`；inline `SectionLabelRow` 抽出至 `components/about/SectionLabelRow.tsx`；hero per BQ-045-05 PM Option A 從 width=narrow(768) 升 1248（K-022 AC-022-HERO-TWO-LINE 已於 K-040 retired，無 Sacred 衝突）；PageHeaderSection 去除 outer `py-20`；K-031 Sacred + K-034 Phase 1 Footer byte-identity + K-040 pairwise ≤2px 全保留；設計文件 [K-045-design.md](../docs/designs/K-045-design.md)
...
│   │           │   ├── PageHeaderSection.tsx             ← S1 One operator 聲明（K-022 三層拆法；K-034 Phase 2 D-19/D-20/D-21 改左對齊 2-line hero ink+brick + 全寬 divider + gap-18 flex column；**K-045 Architect design 2026-04-24 pending** — outer `py-20` 移除，垂直節奏改由 parent `<section id="header">` `pt-8 sm:pt-[72px]` 承接）
│   │           │   ├── SectionLabelRow.tsx               ← **K-045 Architect design 2026-04-24 pending** — 從 AboutPage.tsx inline 抽出為獨立 file；props `{ label: string }`；render Geist Mono 13px bold letterSpacing 2 `<span data-testid="section-label">` + `<div data-section-hairline>` 1px hairline；5 call sites (S2–S6)
...
│   │           │   ├── SectionContainer.tsx               ← P1；/about 6 sections 外層 wrap（**K-045 Architect design 2026-04-24 pending retirement** — BQ-045-02 Option α；Engineer Phase 1 step 3 DELETE；磁碟至 Engineer 交付前仍存在；K-031 adjacency 藉 AboutPage.tsx 改寫 per-section 容器替代，不再需要 primitive 抽象）
