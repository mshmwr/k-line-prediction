...
updated: 2026-04-26 (K-051 Phase 4 Architect — predictor message-vs-gate drift fix + toast `data-testid` + UI i18n design landed; AC-051-10 backend gate at `predictor.py:156` tightens from `< MA_WINDOW = 99` to `< MA_TREND_WINDOW_DAYS + MA_WINDOW = 129`, message at `predictor.py:335` becomes f-string `f"ma_history requires at least {MA_TREND_WINDOW_DAYS + MA_WINDOW} daily bars ending at that date."` (substring runtime byte-identity preserved, K-051 user-retest SOP grep continues to pass); both query-side caller (line 331-336, raises Sacred ValueError) and candidate-side caller (line 343-345, silently `continue`s) move to 129-bar floor in unison per PM B-Phase4-hidden-callsite ruling. AC-051-10 B1 adds 3 new boundary unit tests in `test_predictor.py` after line 603 (128 bars→`[]` / 129 bars→30 floats / 130 bars→30 floats), reusing existing `_make_real_date_1d_bars` helper (no DB I/O). AC-051-10 B2 deletes 11-line stale empirical-floor comment block at `test_predict_real_csv_integration.py:33-43` + 10-line Truncation rationale paragraph; replaces with 5-line coupled-constant block; drift-guard assertion reshapes from `SACRED_FLOOR == MA_WINDOW == 99` to `SACRED_FLOOR == MA_TREND_WINDOW_DAYS + MA_WINDOW == 129`. AC-051-11 adds `data-testid="error-toast"` to `AppPage.tsx:350`; `upload-real-1h-csv.spec.ts:171-173` swaps chained `.text-red-400.border-red-700.bg-red-950` selector for `getByTestId('error-toast')`. AC-051-12 + B4 + B3: 7 source-string i18n edits (`AppPage.tsx:363/379/399` + `MainChart.tsx:264/270` + `PredictButton.tsx:16` + `BusinessLogicPage.tsx:106`) + 6 spec-assertion edits + 1 test-description edit in `ma99-chart.spec.ts` (lines 188/194/238/247/268/274); all full-width punctuation (`（）：，…`) replaced with ASCII (`():,...`); code-internal Chinese (zh-TW timestamp regex `MainChart.tsx:33,38`, JS comments in `UnifiedNavBar.tsx:7-20`, `__tests__/diary.english.test.ts:9-16` CJK regex character class, `frontend/e2e/K-046-example-upload.spec.ts:105` K-046 Sacred assertion, sitewide-fonts/about-v2/sitewide-footer/pages/navbar/_fixtures comments) explicitly preserved per AC-051-12 scope boundary. 0 schema / 0 API endpoint / 0 shared-component / 0 route / 0 Pencil frame change. `MA_WINDOW = 99` + `MA_TREND_WINDOW_DAYS = 30` constants UNCHANGED. Sacred K-015 substring `"ma_history requires at least 129 daily bars ending at that date"` runtime byte-identity preserved (`30 + 99 = 129` interpolated). Worktree hydration drift policy AC-051-06 carries forward. Design doc [K-051-phase4.md](../docs/designs/K-051-phase4.md). 前置：K-051 Phase 3b/3c Architect — permanent regression coverage design landed; AC-051-07 backend pytest `backend/tests/test_history_db_contiguity.py` (NEW ~80 LOC, 3 test functions: full daily DB no-gap sweep + monotonic-ascending date order + ≥129-bar minimum on 2026-04-08 anchor) loads canonical `history_database/Binance_ETHUSDT_d.csv` (3157 lines = URL+header+3155 daily bars 2017-08-17→2026-04-08), reads `MA_TREND_WINDOW_DAYS + MA_WINDOW = 129` import from predictor.py L8/L11 (no magic number); AC-051-08 backend integration test `backend/tests/test_predict_real_csv_integration.py` (NEW ~120 LOC, 3 test functions: positive `test_real_db_real_csv_returns_matches` + negative `test_truncated_db_raises_sacred_value_error` + drift-guard `test_sacred_message_contains_substring`) wires real `find_top_matches(input_bars=load_official_day_csv(fixture), history=load_csv_history(HISTORY_1D_PATH), ma_history=same)` (predictor.py L314 sig + main.py L17 path + mock_data.py L30 parser); negative path Sacred ValueError exact substring `"ma_history requires at least 129 daily bars ending at that date"` matched via `pytest.raises(ValueError, match=re.escape(...))` against truncated 128-bar slice; new fixture `backend/tests/fixtures/ETHUSDT-1h-2026-04-07-original.csv` (24 bars, byte-clean UTF-8, headerless, comma-delimited Unix-ms first column) + sibling README `ETHUSDT-1h-2026-04-07-original.README.md` documenting regeneration command (provenance NOT in CSV — `parseOfficialCsvFile` strict numeric-first-column gate at AppPage.tsx L48-66 throws on `# Source:` comment lines, verified by Pre-Design Audit dry-run); AC-051-09 frontend E2E `frontend/e2e/upload-real-1h-csv.spec.ts` (NEW ~180 LOC, 3 test cases: positive upload→chart-visible→MatchList-populated + negative 23-bar fixture→error-toast `.text-red-400` + AC-051-08-FIXTURE-PARITY byte-identical with backend) follows ma99-chart.spec.ts:163-168 canonical setInputFiles pattern (NOT K-046 spec L97-99 which targets history-reference, NOT official-input — Pre-Design Audit caught the misroute); 3 mocks `/api/history-info` + `/api/merge-and-compute-ma99` + `/api/predict` (NO `/api/predict-stats`, endpoint does not exist per main.py routes inspection — SQ-051-PHASE3-01 resolved within design authority); MOCK_PREDICT_BASE.matches[0].future_ohlc.length === 2 + complete MOCK_STATS PredictStats fields (K-013 mock-realism contract preserved); LIFO mock-apis.ts caller-registers-specific-mock-after-mockApis pattern; new `frontend/e2e/fixtures/` directory (leaf-additive) + `ETHUSDT-1h-2026-04-07.csv` byte-identical with backend fixture (mirrored physical files, no symlink). 0 backend code change / 0 schema change / 0 API endpoint change / 0 shared-component change / 0 route change. Sacred K-015 `ma_history requires ≥129 daily bars ending at input date` gains positive (real-DB pass-through) + negative (truncated-DB ValueError) anchors. Worktree hydration drift policy AC-051-06 carries forward — Engineer canonical re-run before classifying any test failure as regression. Design doc [K-051-phase-3-design.md](../docs/designs/K-051-phase-3-design.md) (449 LOC, 11 sections, truth tables backed by `git show HEAD:` evidence, B1–B5 verification grid, file change list of 10 paths, three-phase implementation order). Architect did not modify ticket AC; B1–B5 PM rulings encoded by PM 2026-04-26 already canonical in ticket text. 0 PM blocking questions, 0 Architect-pending items. 前置：K-050 Engineer — Footer brand-asset SVG anchors + click-to-copy email button landed; supersedes K-034 P1 plain-text framing on /、/about、/business-logic、/diary 四路由 (single zero-prop `<Footer />` 共享 DOM byte-identical 跨 4 路由 K-034 P1 T1 守護)；3 brand-asset SVG (mail / github / linkedin) 經 vite-plugin-svgr `?react` import 為 wrapper component；email `<button>` 含 navigator.clipboard.writeText + range-selection fallback + 1500ms revert + sr-only role=status aria-live=polite 狀態廣播；K-018 AC-018-CLICK Sacred 完整恢復 + 1 cross-route sanity test；K-017 AC-017-FOOTER anchor 部分恢復 (href + testid，"Let's talk →" copy 不恢復)；K-022 italic/underline 不恢復；K-030 /app no-Footer + K-045 T18/T19 width parity + K-040 pairwise ≤2px 全保留；Pencil SSOT 4 frames `1BGtd / 86psQ / ei7cl / 2ASmw` flat-text 為 layout-placeholder，runtime divergence 由新增 `design-exemptions.md §2 BRAND-ASSET` category 背書 (4 frame JSON 各帶 `_design-divergence` / `_designDivergence` 欄位記錄 Pencil flat-text → runtime component 對映)；Route Impact: /、/about、/business-logic、/diary — /app K-030 isolation 不動；0 backend / 0 schema / 0 API change. Pre-existing K-040 narrative bloat 留 TD-K050-03 跟蹤. Designer persona supplement TD-K050-01；Design System Handbook TD-K050-02). 前置：K-046 Architect — /api/upload-history write-path comment-out design landed; BQ-46D/E/F PM-ruled; backend main.py:162-167 write block comment-out + response payload honest to existing authoritative state (bar_count = len(existing), latest = existing[-1], added_count = 0 always); frontend AppPage.tsx gains Download example CSV → anchor linking /examples/ETHUSDT_1h_test.csv (646B, byte-identical copy of history_database/Binance_ETHUSDT_1h_test.csv minus Binance_ prefix); 4 existing backend upload tests augmented with mtime_ns+id()+len() invariants + 2 new tests (AC-046-QA-2 strictly-later reversibility + AC-046-QA-4 example CSV round-trip); new frontend/e2e/K-046-example-upload.spec.ts (3 cases); K-009 ma_history=_history_1d + K-013 stats_contract_cases.json Sacred preserved (no main.py read-path touch); Route Impact Table 5 routes → only /app affected; Shared Component Inventory = none (inline anchor, not primitive); 3 Known Gaps PM-acknowledged (concurrency moot / CDN race ops-time / cross-browser download Chromium-only). 前置：K-045 Engineer — /about desktop layout consistency landed; SectionContainer.tsx primitive DELETED (git rm, 單一 consumer 清空 BQ-045-02 Option α); AboutPage.tsx 改寫為 6 <section> 直接為 root <div className="min-h-screen"> 子元素 (per-section `max-w-[1248px] mx-auto px-6 sm:px-24 w-full` + `mt-6 sm:mt-[72px]` / `pt-8 sm:pt-[72px]` / `mb-8 sm:mb-[96px]` 垂直節奏)；Pencil Y80Iv `padding:[72,96,96,96], gap:72` 對齊；hero BQ-045-05 Option A 1248 full-width 取代 width=narrow (768); K-031 #architecture→<footer> 鄰接保留 (runtime 驗 `document.getElementById('architecture').nextElementSibling.tagName === 'FOOTER'` pass); K-034 Phase 1 Footer byte-identity preserved (T1 byte-identical outerHTML 跨 /、/about、/business-logic、/diary 四路由); K-040 pairwise ≤2px Footer width preserved (T19 跨 /、/about、/diary 三路由 × 1280/1440/375 三 breakpoint 全 Δ=0px); SectionLabelRow 抽至獨立 `components/about/SectionLabelRow.tsx`; PageHeaderSection 去除 outer `py-20`; 19 new Playwright test IDs T1-T19 全 pass (about-layout.spec.ts 15/15 + shared-components.spec.ts T18/T19 新增); Footer snapshot baseline /about + /diary 已同 commit 更新 (docH 3790→3263 subpixel anti-alias 合法 drift, DOM byte-identity 仍由 T1 守護); full Playwright suite 261 pass + 1 pre-existing flaky (AC-020-BEACON-SPA, 已於 HEAD 00f8ac6 驗證同樣 fail, K-020 close commit cd19a75 為 8/9 green) + 1 skipped. Engineer commit 序列: 0b6567d [Phase 1] + 55f640c [Phase 3] + Phase 4 (this commit). 前置：K-044 Architect — README showcase rewrite docs-migration design landed; new `## Deployment Architecture` section sourced from ex-README L147–156 + CLAUDE.md Deploy Checklist cross-ref; `## Data Flow` top gains 5-step user-narrative prose intro sourced from ex-README L84–89. 前置：K-040 Item 1 Architect — sitewide typography reset design landed; Bodoni Moda + Newsreader italic → Geist Mono italic OFF sitewide; tailwind.config.js fontFamily.display/italic 鍵退役待 Engineer 交付同 commit 落地；body @apply font-mono；18 tsx edit sites + timelinePrimitives + K-024 visual-spec.json 原子同 commit；4 stale Sacred E2E block rewrite + 5 new test IDs；Footer + UnifiedNavBar + FileNoBar + SectionHeader K-034 Sacred + already-mono 無異動；/app K-030 isolation screenshot parity 硬 gate. 前置：K-034 Phase 3 Architect — /diary shared Footer adoption design landed; K-017 AC-017-FOOTER /diary neg + K-024 /diary no-footer Sacred + K-034 Phase 1 T4 AC-034-P1-NO-FOOTER-ROUTES /diary row 三條全退役；/app K-030 isolation preserved 不動；TD-K034-P3-02 opened 640–768px Footer viewport seam as Known Gap; shared Footer 消費路由 3 → 4；design doc [K-034-phase3-diary-footer-adoption.md](../docs/designs/K-034-phase3-diary-footer-adoption.md). 前置：K-034 Phase 2 Architect — /about full visual audit design landed; 27 drifts remediation plan; FileNoBar primitive added; DossierHeader retirement; architecture.md synced in-session. K-037 closed — favicon wiring + manifest. K-034 Phase 1 Engineer implementation still pending PM ruling on BQ-034-P1-01.)
---
...
│   │           ├── about/                        ← K-017 大幅重構（2026-04-19）；K-022 結構細節 v2（2026-04-21）；K-034 Phase 2 Pencil SSOT 全頁對齊（2026-04-23）
│   │           │   ├── FileNoBar.tsx                     ← K-034 Phase 2 新增；dark charcoal header bar（bg-[#2A2520] text-paper Geist Mono 10px letterSpacing 2 padding [6,cardPadding]）；5 card consumers（Metric/Role/Pillar/TicketAnatomy/Arch）；props { fileNo: string, rightSlot?: ReactNode, variant?: 'FILE'|'LAYER'|'CASE', cardPaddingSize?: 'md'|'lg' }；約束 padding 跟隨 CardShell size（防 silent drift）
...
│   │           │   ├── SectionLabelRow.tsx               ← **K-045 2026-04-24 landed** — 從 AboutPage.tsx inline 抽出為獨立 file；props `{ label: string }`；render Geist Mono 13px bold letterSpacing 2 `<span data-testid="section-label">` + `<div data-section-hairline>` 1px hairline；5 call sites (S2–S6)
...
│   │           │   ├── RoleCardsSection.tsx              ← S3 6 roles 容器（K-034 Phase 2：刪 h2 "The Roles" + subtitle；intro 改 Pencil s3Intro em-dash literal；ROLES data 刪 `redactArtefact` 欄）
│   │           │   ├── RoleCard.tsx                      ← K-034 Phase 2 重寫；interface `{ role, owns, artefact, fileNo }`；刪 POSITION/BEHAVIOUR ROLE_ANNOTATIONS const + marginalia + redactArtefact；加 FileNoBar (PERSONNEL)；40px charcoal rule；role font-size Bodoni 36/32 by length（`role.length<=2 ? 36 : 32`）；OWNS label Geist Mono 10 + Newsreader italic 14 text
...
│   │           │   ├── TicketAnatomySection.tsx          ← S5 K-002/K-008/K-009 trio 容器（K-034 Phase 2：刪 h2 + subtitle；intro 改 Pencil s5Intro literal；TICKETS outcome/learning 文字不變）
