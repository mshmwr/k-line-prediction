...
**已知風險（TD-003）：** 用 module globals（`_history_1h` / `_history_1d`）做 read-merge-write-swap，無同步機制，併發上傳可能遺失 bars。**Post-K-046 write path 註解後 race surface 移除**，直到 K-047 重啟 write path 再回到此風險面；revisit 於 K-047 Architect design phase。
...
| `/diary` | `<Footer />`（**K-034 Phase 3 design 2026-04-23**：為 root `<div className="min-h-screen">` 最末 sibling；K-017 AC-017-FOOTER /diary 負斷言 + K-024 /diary no-footer Sacred + K-034 Phase 1 T4 AC-034-P1-NO-FOOTER-ROUTES /diary row 三條 Sacred 全退役；/app K-030 isolation preserved 不動 per BQ-034-P3-03；Pencil provenance 沿用 86psQ + 1BGtd 無新 frame，BQ-034-P3-01；640–768px viewport padding seam 為 Known Gap TD-K034-P3-02；pre-K-034-P3 為「無 footer（K-024 Architect 裁決）」） |
...
| `Footer` | `components/shared/Footer.tsx` | `/` / `/about` / `/business-logic` / `/diary` **四路由** render 同一 zero-prop `<Footer />`（K-034 Phase 3 design 2026-04-23 加入 /diary，AC-034-P3-DIARY-FOOTER-RENDERS；K-034 Phase 1 design 2026-04-23 retire variant prop；Pencil SSOT = frames 86psQ + 1BGtd byte-identical inline one-liner, Designer 8d95c03；/diary Pencil provenance 沿用 86psQ + 1BGtd 無新 frame per BQ-034-P3-01）；pre-K-034 為 K-035 `variant: 'home' \| 'about'` props（α-premise empirically 證偽）；Engineer 實作 pending PM BQ-034-P1-01 ruling（Phase 1 scope）+ K-034 Phase 3 /diary 納入（此本次）。/app K-030 isolation preserved 不渲染 per AC-030-NO-FOOTER Sacred（distinct rationale，Phase 3 未退役此 Sacred） |
