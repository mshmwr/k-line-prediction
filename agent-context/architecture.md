...
updated: 2026-04-21 (K-013 Architect design)
---
...
**Known Gap（K-013 設計階段記錄，PM 知情）：** 後端 `PredictStats.consensus_forecast_1h/1d` 目前永遠是 `[]`（`compute_stats` 從未填入；`models.py` 預設 `[]`）。全集分支（`appliedSelection == all matches`）直接用 `appliedData.stats`，`StatsPanel::ConsensusForecastChart` 收到空陣列 → 全集下**無 consensus 圖**；subset 分支前端由 `AppPage.tsx` 將 `projectedFutureBars` 注入 `displayStats.consensusForecast1h/1d`，才有圖。此為 K-013 之前既有行為，不在本票 scope；若要讓全集也顯圖，需另開 ticket 決定（a）後端補算並填 `consensus_forecast_*`，或（b）前端全集分支也呼叫 `computeStatsFromMatches`。K-013 設計文件 §0 SQ-013-01 記錄此資訊。
