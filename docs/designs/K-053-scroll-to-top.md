...
3. **Changelog section** (line 681) — prepend new entry:
   ...
   ```markdown
   - **2026-04-26** (Architect, K-053 design) — `ScrollToTop` sitewide scroll-reset on route change designed. New component `frontend/src/components/ScrollToTop.tsx` (18 LOC: `useLocation` + `useEffect([pathname, hash])` + `if (hash) return` early-return + `window.scrollTo({top:0, left:0, behavior:'instant'})`, returns `null`); mount point after `frontend/src/main.tsx:34` (`<GATracker />` sibling, OUTSIDE `<Routes>` + OUTSIDE `<Suspense>`); new Playwright `frontend/e2e/scroll-to-top.spec.ts` (3 cases: T-K053-01 `/diary`→`/about` reset / T-K053-02 hash early-return preserves scroll / T-K053-03 same-route skipped pending Phase 1 PM ruling). Pre-Design Audit: `grep -rn "scrollY\|scrollTo" frontend/src/` 0 hits (baseline has no scroll-orchestration code); `grep -rn "useLocation" frontend/src/` 1 hit (`useGAPageview.ts`, the only existing mirror pattern); no Sacred clause covers scroll behavior (K-030 / K-034-P1 / K-024 / K-031 all orthogonal); 5 routes (/, /app, /about, /diary, /business-logic) regression sweep all pass (`/app` `h-screen overflow-hidden` makes scrollTo no-op, preserving K-030 isolation). Edge case truth table 16 rows (pathname × hash × search × initial-mount × POP/PUSH × StrictMode) — rows #8 (same-route) + #9/#10 (back/forward) are `AC-K053-06` PM ruling slots; row #5 (hash removal) + row #11 (refresh-mid-scroll) are Architect default-decisions, reversible by Phase 1 QA. 0 backend / 0 schema / 0 API / 0 props interface / 0 CSS / 0 Pencil frame changes. Cross-page duplicate audit 0 hit; Sacred 7-pattern grep sweep 0 hit; regression analysis confirms the only two `window.scrollY` uses in `frontend/e2e/about-layout.spec.ts` are read-only geometric calc, unaffected. Same-Session Verdict Reservation declared; Engineer Design Challenge Sheet ≤2 turns response. Design doc: [K-053-scroll-to-top.md](../docs/designs/K-053-scroll-to-top.md). No code changed (Architect design only).
   ```
