...
- 3-file CJK violation fix (commit `c218fc3`): `docs/designs/shared-components-inventory.md:11` (single-char CJK suffix on `Banner` replaced with `Banner, etc.`), `docs/designs/K-053-scroll-to-top.md:595` Architect changelog paragraph (full English rewrite), `docs/tickets/K-051-daily-db-backfill-rollup-fix.md` (9 CJK lines paraphrased per new `feedback_retro_no_verbatim_user_quotes.md` rule — verbatim user quotes converted to English lesson statements; CJK regex literals replaced with description + line-anchor reference).
