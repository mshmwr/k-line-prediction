...
|---|-------|----------|-----------|
| Q1 | SSOT architecture | **C** — `.pen` is SSOT; Designer exports key-property JSON snapshot into `docs/designs/K-XXX-*.md` AND full frame JSON dump into `frontend/design/specs/<page>.frame-<id>.json` (git-tracked) | "MCP takes too much time, having everyone read it is not cost-effective. Must ensure that when designers modify .pen, they always update the json file" |
...
| D-4 | S3 RoleCard (all 6 cards) | Marginalia POSITION/BEHAVIOUR labels not in Pencil | Pencil `role_*` body children (6 cards): ONLY `OWNS` label + owns text + `ARTEFACT` label + artefact text (no POSITION/BEHAVIOUR, no bottom divider) | Code `RoleCard.tsx:54-64`: renders `<div className="mt-3 pt-2 border-t">` + Geist Mono 9px POSITION or BEHAVIOUR annotation driven by `ROLE_ANNOTATIONS` map | **code-side** | Pencil SSOT has NO marginalia; code adds A-11 labels with no design backing. Remove `ROLE_ANNOTATIONS` map + marginalia div + `annotation` prop entirely. A-11 label motif was designer's pre-Phase-2 intent but never drawn into Pencil → implementation-without-Pencil-backing. **K-039 SPLIT (2026-04-24):** D-4 splits into D-4a (visual — marginalia layout removal, Pencil SSOT) + D-4b (text — `role` / `owns` / `artefact` runtime copy; `content/roles.json` is SSOT, Pencil text = frozen-at-session snapshot). See `feedback_content_ssot_split.md`. |
| D-5 | S3 RoleCard Reviewer card | Over-redacted — Pencil shows Reviewer ARTEFACT text `"Review report + Reviewer Reflection"` plainly visible; code hides it behind RedactionBar | Pencil `r4Art.content: "Review report + Reviewer Reflection"` rendered as Geist Mono 12px text_ink (identical visibility to other 5 roles) | Code `RoleCardsSection.tsx:26`: `redactArtefact: true` → RedactionBar replaces text via `<sr-only>` pattern | **code-side** | Pencil SSOT has zero redaction on Reviewer artefact (nor on any role card). Fix: set `redactArtefact: false` on Reviewer; remove `redactArtefact` prop (always plain). RedactionBar primitive preserved for MetricCard m2 per D-3. |
...
- **When** Engineer implements
- **Then** Reviewer entry `redactArtefact` is `false` (or prop removed entirely); ARTEFACT text `"Review report + Reviewer Reflection"` renders as plain Geist Mono 12px ink text (no RedactionBar, no sr-only)
...
- K-021 Sacred `/about keep FooterCtaSection (K-017 locked)` — already retired by K-035; remains retired.
...
**Next time improvement:** (1) Bump TD-K030-03 priority Medium → High and move into next-in-queue for tooling fix (throw on missing `TICKET_ID` in `visual-report.ts`). (2) Add post-step filename verification gate to `qa.md` §Sign-off stage — `ls docs/reports/K-${TICKET_ID}-visual-report.html` must succeed AND `ls docs/reports/K-UNKNOWN-visual-report.html` must fail; either violation = sign-off BLOCKED. Codified in persona as hard step this session per `feedback_retrospective_codify_behavior`.
