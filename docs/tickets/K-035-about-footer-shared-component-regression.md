...
- [x] Phase 3 — Architect design doc → [`docs/designs/K-035-shared-component-migration.md`](../designs/K-035-shared-component-migration.md) (2026-04-22; ~830 lines; OQ-1 resolved to Option α by weighted scoring 9.7/6.25/5.3; behavior diff 17/0 equivalent across both variants; Route Impact 5 routes / 3 affected / 1 unaffected / 1 isolated; 14 files / 11 Engineer steps; K-021 Sacred `/about retains FooterCtaSection` formally retired in design doc §10; architecture.md synced + self-diff; PM Phase Gate 2026-04-22 PASS — Option α ruling + K-021 Sacred retire both approved per `feedback_pm_self_decide_bq.md` priority-1 Pencil source-of-truth signal)
...
- **PM** had NavBar named explicitly as a shared component in AC-017-NAVBAR but Footer was codified as a page-differentiated inline component across three routes; the K-017 "Design Decision Log" even contradicted its own AC ("Footer CTA is a sitewide shared component" vs three inline footers), and no Phase Gate checked "did this AC name the shared component from `frontend/src/components/`."
