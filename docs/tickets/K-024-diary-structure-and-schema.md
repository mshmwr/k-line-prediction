...
5. **Ticket ID 前綴**：每條 entry 的 ticket title 前顯示 `K-XXX · ` 格式（若 `ticketId` 存在）
...
**Then** 顯示 **3 條** diary entry（非更多、非更少）
...
**And** 左側有 1px 垂直 rail 貫穿所有 entry
**And** rail 顏色為 `rgb(42, 37, 32)`（`#2A2520` = `charcoal`）
**And** 每條 entry 左側與 rail 交接處有磚紅矩形 marker
**And** marker `backgroundColor` 為 `rgb(156, 74, 59)`（`#9C4A3B` = `brick-dark`）
**And** Playwright 斷言：`/diary` 頁面無 `<details>` / `<summary>` 元素（accordion 機制）；有 rail 元素（例 `.diary-rail` 或 `<div>` with `border-left`）
**And** Playwright 斷言：至少 5 個 marker 元素（若 diary.json ≥ 5 筆）
...
**When** 頁面滾動至任一 entry
**Then** entry 含三層文字：
- Ticket title：字型 Bodoni Moda italic，`fontWeight` bold（600+），字級 18px
- Date：字型 Geist Mono，字級 12px
- Text：字型 Newsreader italic，字級 18px
**And** 若 `ticketId` 存在，ticket title 前綴含 `K-XXX · `（例 `K-017 · /about portfolio-oriented...`）
**And** 若 `ticketId` 不存在，無前綴
**And** Playwright 斷言：至少一個 entry 的 title 元素 computed `fontFamily` 含 "Bodoni Moda"、`fontStyle` = `italic`、`fontSize` = `18px`
**And** Playwright 斷言：至少一個 entry 的 date 元素 computed `fontFamily` 含 "Geist Mono"、`fontSize` = `12px`
**And** Playwright 斷言：至少一個 entry 的 text 元素 computed `fontFamily` 含 "Newsreader"、`fontStyle` = `italic`、`fontSize` = `18px`
...
**And** 大標文字為 `Dev Diary`（或設計稿指定文案；Architect 從 frame `wiDSi` 提取確認）
**And** 大標字型為 Bodoni Moda italic，字級 64px（computed `fontSize` = `64px`、`fontFamily` 含 "Bodoni Moda"、`fontStyle` = `italic`）
**And** 大標下方有 1px 水平分隔線（`charcoal` `#2A2520`）
**And** 分隔線下方有 italic 副標（字型 Newsreader italic，內容由 Architect 從設計稿提取）
**And** Playwright 斷言：Hero 區 `<h1>` computed `fontSize` = `64px`、`fontFamily` 含 "Bodoni Moda"
...
**When** 頁面載入完成
**Then** content 容器的 computed `maxWidth` 為 `1248px`
...
- 磚紅矩形 marker 的精確尺寸（Architect 從 frame `wiDSi` 提取）
- Hero 副標文案與大標精確文字
