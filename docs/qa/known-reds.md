...
- `frontend/e2e/shared-components.spec.ts` :: `T-COPY-RAPID — second click within revert window resets timer (useRef cleanup)` — known-red since `K-075` (2026-05-02); reason: test relies on sub-100ms click timing that is environment-sensitive; pre-existing failure confirmed in canonical (not introduced by K-075); remediation: `deferred` — open a timing-stabilization ticket to replace wall-clock assertion with deterministic fake-timer approach.
- `frontend/src/__tests__/diary.legacy-merge.test.ts` :: `AC-024-LEGACY-MERGE — legacy entry constraints > legacy entry text word count is within 50–100` — known-red since `K-075` (2026-05-02); reason: a legacy diary entry's word count dropped to 19 (below floor of 50) in a prior content edit; pre-existing failure confirmed in canonical (not introduced by K-075); remediation: `deferred` — investigate which entry regressed and correct in a docs-only follow-up.
