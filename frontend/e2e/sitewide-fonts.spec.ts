...
// And:   HomeFooterBar（全站 footer 共用組件）computed fontFamily 含 "Geist Mono"
//
...
test.describe('AC-021-FONTS — font-mono class renders Geist Mono', () => {
  ...
  test('HomePage HomeFooterBar font-mono element computed fontFamily 含 "Geist Mono"', async ({ page }) => {
    await mockApis(page)
    await page.goto('/')
    await page.waitForLoadState('networkidle')

    // HomeFooterBar `<footer className="font-mono ...">` 的資訊列是套用 font-mono 的代表元素
