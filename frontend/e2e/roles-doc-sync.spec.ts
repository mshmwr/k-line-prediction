 ...
 *
 * Enforces that the TSX pure-data module `frontend/src/components/about/roles.ts`
 * is the single source of truth for role/owns/artefact text, and that the two
 * downstream Markdown tables (README.md + docs/ai-collab-protocols.md) remain
 * byte-aligned with it between the `<!-- ROLES:start -->` / `<!-- ROLES:end -->`
 * markers. Covers:
 *
 *   - AC-039-P1-ROLES-COUNT-INVARIANT: ROLES.length === 6
 *   - AC-039-P1-TSX-CANON: every TSX `owns` + `artefact` matches /^[^|\n`]*$/
 *   - AC-039-P1-README-SYNCED: marker-delimited README table deep-equals ROLES
 *   - AC-039-P1-PROTOCOLS-SYNCED: marker-delimited protocols.md table deep-equals ROLES
...
import { ROLES, type RoleEntry } from '../src/components/about/roles.ts'
