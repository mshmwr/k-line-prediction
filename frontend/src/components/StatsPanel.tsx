      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={label} className="bg-gray-800 rounded p-3 text-center">
            ...
            <div className={`text-lg font-bold font-mono ${color}`}>{value.toFixed(2)}</div>
          </div>
        ))}
      </div>
      <div className="flex gap-4 text-sm">
        <span className="text-gray-400">Win Rate: <span className="text-white">{(stats.winRate * 100).toFixed(1)}%</span></span>
        <span className="text-gray-400">Avg r: <span className="text-white">{stats.meanCorrelation.toFixed(4)}</span></span>
      </div>
    </div>
  )
}
