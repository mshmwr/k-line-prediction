...
export default function LoadingSpinner({ label }: Props = {}) {
  return (
    <div role="status" aria-label={label ?? 'Loading'} className="flex flex-col items-center justify-center gap-3 rounded-lg bg-[#0D1117] px-8 py-6">
      ...
      {label && <p className="text-zinc-500 font-mono text-[12px]">{label}</p>}
    </div>
  )
}
